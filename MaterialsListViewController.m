...
@interface MaterialsListViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,UICollectionViewDataSource,UICollectionViewDelegate,TYFSencondViewControllerDelegate>
...
@property (strong, nonatomic) IBOutlet UIButton *myXLK;//下拉框
...
//    NSLog(@"############%@",self.guanjianzi);
    ...
    [self RegisterBaseTableView];//注册两个TableView
    [self loadBaseData];//加载BaseTableView数据
    
    [self.myXLK setTitle:@"全部分类" forState:UIControlStateNormal];
...
//        [self loadData:[@"板材类" stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
    ...
    [self.BaseTableVIew scrollToNearestSelectedRowAtScrollPosition:indexPath animated:YES];
    [self.BaseTableVIew selectRowAtIndexPath:indexPath animated:NO scrollPosition:UITableViewScrollPositionNone];
...
//    [self loadData:[@"板材类" stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
    self.myXLK.layer.cornerRadius = 5;
    self.myXLK.layer.masksToBounds = YES;
    [self createView];
...
-(void)createCollectionView
{
    ...
    flowLayout.itemSize = CGSizeMake(70, 40);//设置cell大小
    flowLayout.minimumInteritemSpacing = 10;//最小列间距 默认值 是10
//    flowLayout.minimumLineSpacing = 50;//最小行间距
    
    //    flowLayout.scrollDirection = UICollectionViewScrollDirectionHorizontal ;//设置滚动方向 为水平方向
    
    //创建 collectionview 需要指定大小 还需要指定 它的布局方式
    
    CGRect rect = self.myXLK.frame;
//    self.myView = [[UIView alloc]initWithFrame:CGRectMake(rect.origin.x, rect.origin.y  + rect.size.height +5, 250, 150)];
    
    self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(rect.origin.x, rect.origin.y  + rect.size.height +5, 240, 180) collectionViewLayout:flowLayout];
    
    
    self.collectionView.dataSource = self;//数据源代理
    self.collectionView.delegate = self;//视图交互相关的代理
    self.collectionView.backgroundColor = [UIColor grayColor];
    //注册 cell 对应标志
    [self.collectionView registerNib:[UINib nibWithNibName:@"myCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:@"cellID"];
    [self.myView addSubview:self.collectionView];
}



#pragma mark - 实现UICollectionView代理
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    myCollectionViewCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"cellID" forIndexPath:indexPath];//先去复用池找cell没有找到空闲的cell 就帮你 创建 如果找到了 直接返回
    cell.backgroundColor = [UIColor colorWithRed:20/256.0 green:150/256.0 blue:124/256.0 alpha:1];
    cell.myLabel.text = [self.ArrayCollection[indexPath.item] clxlmc];
    return cell;
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self.myXLK setTitle:[self.ArrayCollection[indexPath.item] clxlmc] forState:UIControlStateNormal];

    
    if ([[self.ArrayCollection[indexPath.item] clxlmc] isEqualToString:@"奥松板"]||[[self.ArrayCollection[indexPath.item] clxlmc] isEqualToString:@"密度板"]||[[self.ArrayCollection[indexPath.item] clxlmc] isEqualToString:@"欧松板"]||[[self.ArrayCollection[indexPath.item] clxlmc] isEqualToString:@"水泥压力板"])
    {
        ...
        NSString *ss = @"%E6%9D%BF%E6%9D%90%E7%B1%BB";
        [self.request requestWithUrl:[NSString stringWithFormat:@"http://111.200.41.13:8090/sgdmm?clxlmc=%@&cldlmc=%@&funname=clbmSj",[[self.ArrayCollection[indexPath.item] clxlmc] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],ss] parameters:nil complicate:^(BOOL success, NSString * object) {
            if (success) {
                ...
                int j = 5;
                for (NSInteger i = 0; i<=firstElementArr.count/5-1; i++) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == self.BaseTableVIew) {
        ...
        cell.selectedBackgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 10, 10)];
        cell.selectedBackgroundView.backgroundColor = [UIColor colorWithRed:20/256.0 green:150/256.0 blue:124/256.0 alpha:1];
        

        cell.backgroundColor = [UIColor groupTableViewBackgroundColor];
        cell.layer.borderWidth = 1;
        cell.layer.borderColor = [[UIColor whiteColor]CGColor];
        cell.textLabel.text = [self.BaseDataSource[indexPath.row] cldlmc];
